...
import { renderOceanFauna, renderWhaleGlimmer } from "./ocean-fauna";
...
function renderOcean(context: CanvasRenderingContext2D, grid: OceanGrid, time: number) {
  ...
  renderOceanFauna(context, { width, height }, time);
  ...
  renderWhaleGlimmer(context, { width, height }, time);
...
export function AsciiOcean() {
  ...
  useEffect(() => {
    ...
    const animate = (now: number) => {
      ...
      const compact = (grid?.width ?? window.innerWidth) < 560;
      const preferredFrameRate = compact ? 28 : 36;
      let frameRate = preferredFrameRate;
      if (averagePaintTime > 15) frameRate = compact ? 20 : 24;
      else if (averagePaintTime > 10) frameRate = compact ? 24 : 30;
